...
int check_table_indices_update() {
    ...
    allocate_sector(file, sectors_needed, -1, -1);
    ...
    // stub creating table
    uint32_t table1_schema = allocate_sector(file, 10, -1, -1);
    uint32_t table2_schema = allocate_sector(file, 20, -1, -1);

    add_table_index(header.table_indices_sector, file, "table1", table1_schema, global_index_array);
    add_table_index(header.table_indices_sector, file, "table2", table2_schema, global_index_array);
    ...
    uint32_t table3_schema = allocate_sector(file, 20, -1, -1);
    remove_table_index(header.table_indices_sector, file, "table2", global_index_array);
    update_table_sector_link(header.table_indices_sector, file, "table1", table3_schema, global_index_array);
    read_table_index_from_sector(file, header.table_indices_sector, &copy_from_file);
    printf("Modified ");
    debug_table_indices(&copy_from_file);
